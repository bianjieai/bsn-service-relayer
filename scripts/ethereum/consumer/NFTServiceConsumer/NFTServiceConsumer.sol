...
contract NFTService is iServiceClient {
    ...
    string private priceRequestInput = "{\"pair\":\"usdt-eth\"}"; // price request input
        ...
        returns (string memory)
    {
        ...
        (returnValue, tokens, actualNum) = JsmnSolLib.parse(_output, 1);
    ...
    function _buildMintRequest(
        ...
        returns (string memory)
    {
        string memory abiEncoded = string(abi.encodePacked(_to, amount, _metaID, _setPrice, _isForSale));
        return strConcat(strConcat('{"abi_encoded":"', abiEncoded),'"}');
    }
